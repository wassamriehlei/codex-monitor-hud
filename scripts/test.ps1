...
foreach ($required in @('CompletionSoundCombo','CompletionSoundOffItem','CompletionSoundAsteriskItem','CompletionSoundExclamationItem','CompletionSoundBeepItem','CompletionSoundPreviewButton')) { if ($settingsXaml -notmatch [regex]::Escape($required)) { throw "Completion-sound setting '$required' is missing." } }
...
foreach ($required in @('sourceDesktop','sourceCliOpenAI','sourceCliDeepSeek','GetSourceGeometry','ProfileId','ClientSurface','ModelProvider')) { if ($compiledSourceText -notmatch [regex]::Escape($required) -and $mainText -notmatch [regex]::Escape($required)) { throw "Desktop/CLI source distinction '$required' is missing." } }
...
if ($compiledSourceText -notmatch [regex]::Escape('bubble.DismissRequested += path => SetDetached(path, false') -or $compiledSourceText -match [regex]::Escape('bubble.DismissRequested += path => DismissRequested?.Invoke(path)') -or $mainText -notmatch '\$entry\.Dismiss\.Add_Click\(\(\{ Set-SessionDetached \$taskPath \$false') { throw 'Detached-bubble close must remove only that projection while main monitoring continues.' }
...
foreach ($required in @('Invoke-HudCompletionSound','CompletionSoundPreviewButton','System.Media.SystemSounds')) { if ($mainText -notmatch [regex]::Escape($required)) { throw "Completion-sound runtime path '$required' is missing." } }
foreach ($required in @('CompletionRevision','PlayCompletionSoundForTransitions','System.Media.SystemSounds')) { if ($compiledSourceText -notmatch [regex]::Escape($required)) { throw "Compiled completion-sound path '$required' is missing." } }
