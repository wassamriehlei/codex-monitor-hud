...
internal sealed partial class HudApplicationController : IDisposable
{
    ...
    public void Start()
    {
        _log.Write($"Compiled HUD v3.4.2 starting. config={_paths.ConfigPath}; profiles={string.Join(',', _profiles.Select(static profile => profile.Id))}; agentNotices={_settings.AgentNotifications.Enabled}/{_settings.AgentNotifications.Permission}");
